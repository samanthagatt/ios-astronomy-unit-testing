...
class MockDataLoader: NetworkDataLoader {
    ...
    func loadData(from request: URLRequest, completion: @escaping (Data?, Error?) -> Void) {
        ...
        // runs code inside closure one second in the future
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            completion(self.data, self.error)
        }
    }
...
class AstronomyTests: XCTestCase {
    ...
    func testFetchPhotosWithValidData() {
        ...
        let solDescriptions = SolDescription(sol: 4, totalPhotos: 9, cameras: ["Hi"])
        let marsRover = MarsRover(name: "Curiosity", launchDate: Date(), landingDate: Date(), status: .active, maxSol: 3, maxDate: Date(), numberOfPhotos: 90, solDescriptions: [solDescriptions])
        
        
        let mock = MockDataLoader(data: testJSON)
        let marsRoverClient = MarsRoverClient(dataLoader: mock)
        
        marsRoverClient.fetchPhotos(from: marsRover, onSol: 0) { (photoRefs, error) in
